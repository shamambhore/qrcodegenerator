...
def qr_generate(request):
    if request.method == 'POST':
        data = request.POST['data']
        logo = Image.open('/home/test/QRCodeGenerator/qrcodesite/images/microbiome.png')
        basewidth = 200
        ...
        QRcode.make()
        QRimg = QRcode.make_image(image_factory=StyledPilImage, module_drawer=VerticalBarsDrawer(), color_mask=SquareGradiantColorMask(), eye_drawer=RoundedModuleDrawer())
